...
def regress_bh(params):
    ...
    c = odeint(monod, initial_states, times_m, args=(umax, Km, Yxs)) #, Yps, Yxs))
...
def regress( params ):
    ...
    # Model prediction
    c = odeint(monod, initial_states, times_m, args=(umax, Ks, Yxs))
    cX = c[:, 0]
    # cS = c[:, 1]
    # cP = c[:, 2]
    del c
